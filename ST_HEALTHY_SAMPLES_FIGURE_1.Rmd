...
### Version 1: Batch correction after correcting SCT counts.

```{r}
healthy_skin.merge <- merge(x=Healthy_Samples[[1]],y=Healthy_Samples[-1])

VariableFeatures(healthy_skin.merge) <- all.variable.features %>% unique()
...
#healthy_skin.merge <- SCTransform(healthy_skin.merge,assay = "Spatial")
all_healthy_samples.hm.sct <- RunHarmony(healthy_skin.merge,assay.use = "SCT",project.dim = FALSE,group.by.vars = "sample.id")
```

```{r}
all_healthy_samples.hm.sct.v1 <- all_healthy_samples.hm.sct %>% 
...
DimPlot(all_healthy_samples.hm.sct.v1,cols = custom_colors,pt.size = 3.5)
#dev.off()
DimPlot(all_healthy_samples.hm.sct.v1,cols = color.labels,pt.size = 3.5)
```
```{r}
...
names(cluster.labels) <- levels(all_healthy_samples.hm.sct.v1)
all_healthy_samples.hm.sct.v1 <- RenameIdents(all_healthy_samples.hm.sct.v1, cluster.labels)
all_healthy_samples.hm.sct.v1 <- StashIdent(all_healthy_samples.hm.sct.v1, save.name = "Spatial.regions")
```

```{r}
custom_colors_v1 <- c("#B4DFFC","#6EAB3D","#FFD700","#A020F0","#FFA500","#AEDD3C","#595959","#D2AF81FF","#3A49FC")
show_col(custom_colors_v1)
```

#Define color scheme
```{r}
custom_colors <- c("#B4DFFC","#6EAB3D","#FFD700","#A020F0","#FFA500","#AEDD3C","#595959","#D2AF81FF","#3A49FC","#FF0000","#A86F3D","#A18FAA")
color.labels <- c("0 Dermis fibroblasts"="#B4DFFC","1 Mixed"="#6EAB3D","2 Dermis adipose, connective tissue"="#FFD700","3 Macrophages"="#A020F0","4 Epidermis 1"="#FFA500","5 Smooth muscle cells"="#AEDD3C","6 Epidermis 2"="#595959","7 Melanocytes"="#D2AF81FF","8 Dermal lymphatics"="#3A49FC")
```
...
images <- Images(all_healthy_samples.hm.sct.v1) %>% as.vector()
for(x in images){
  pdf(width = 8,height=10,file = paste("HM_CORRECTED_HEALTHY_SAMPLES/",x,"_(HM_CORRECTED)_HEALTHY_SPATIAL_PLOT.pdf"))
  print(SpatialDimPlot(all_healthy_samples.hm.sct.v1,images=x,cols = color.labels,pt.size.factor = 2))
  dev.off()
}
```
```{r}
all_healthy_samples.hm.sct.v1 <- PrepSCTFindMarkers(all_healthy_samples.hm.sct.v1,assay = "SCT")
Idents(all_healthy_samples.hm.sct.v1) <- "seurat_clusters"
all_healthy_samples.hm.sct.v1.markers <- FindAllMarkers(all_healthy_samples.hm.sct.v1, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25,assay = "SCT")
DefaultAssay(all_healthy_samples.hm.sct.v1) <- "SCT"
top8 <- all_healthy_samples.hm.sct.v1.markers %>%
    dplyr::group_by(cluster) %>%
    dplyr::filter(p_val_adj<0.05) %>%
    top_n(n = 8, wt = avg_log2FC)
pdf(width = 20,height=15,file = "HEATMAP_HEALTHY_SAMPLES_(HM_VER_1).pdf")
print(DoHeatmap(all_healthy_samples.hm.sct.v1, features = top8$gene,assay = "SCT",group.colors = custom_colors) + NoLegend())
dev.off()
```
...
```{r figure-1d,fig.height=10,fig.width=10}
Regions.df <- table(all_healthy_samples.hm.sct.v1@meta.data$Spatial.regions,all_healthy_samples.hm.sct.v1@meta.data$sample.id) %>% as.data.frame() %>% dplyr::rename(Spatial_Region=Var1,Sample=Var2) 
...
ggplot(Regions.df,aes(x=Sample,y=Freq,fill=Spatial_Region)) + geom_bar(stat="identity",position="fill") + scale_fill_manual(values =color.labels)  + ggplot2::theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(),axis.text.x =black.bold.16.text) + scale_y_continuous(breaks = brks, labels = scales::percent(brks)) + ylab("% Composition of Sample by Spatial Region / Clusters")
dev.off()
```
...
all_healthy_samples.hm.sct.v2 <- RunHarmony(healthy_skin.merge,assay.use = "SCT",project.dim = FALSE,group.by.vars = "sample.id")
```

```{r}
all_healthy_samples.hm.sct.v2 <- all_healthy_samples.hm.sct.v2 %>% 
    RunUMAP(reduction = "harmony", dims = 1:40) %>% 
    FindNeighbors(reduction = "harmony", dims = 1:40) %>% 
...
```{r}
pdf(width = 12,height=8,file = "UMAP_HEALTHY_SAMPLESall_(HM_VER_2).pdf")
DimPlot(all_healthy_samples.hm.sct.v2,cols = custom_colors,pt.size = 3.5)
dev.off()
```

```{r}
all_healthy_samples.hm.sct.v2 <- PrepSCTFindMarkers(all_healthy_samples.hm.sct.v2,assay = "SCT")
Idents(all_healthy_samples.hm.sct.v2) <- "seurat_clusters"
all_healthy_samples.hm.sct.v2.markers <- FindAllMarkers(all_healthy_samples.hm.sct.v2, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25,assay = "SCT")
DefaultAssay(all_healthy_samples.hm.sct.v2) <- "SCT"
top8 <- all_healthy_samples.hm.sct.v2.markers %>%
    dplyr::group_by(cluster) %>%
    dplyr::filter(p_val_adj<0.05) %>%
    top_n(n = 8, wt = avg_log2FC)
pdf(width = 20,height=15,file = "HEATMAP_HEALTHY_SAMPLES_(HM_VER_2).pdf")
print(DoHeatmap(all_healthy_samples.hm.sct.v2, features = top8$gene,assay = "SCT",group.colors = custom_colors) + NoLegend())
dev.off()
```
