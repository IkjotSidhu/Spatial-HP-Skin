---
title: "CIBERSORT DerM22 cell type deconvolution"
subtitle: "ST"
date: "`r Sys.Date()`"
output:
  html_notebook:
...
```{r}
out_dir = "./out-celltypes-cibersort-derm22"
...
```{r message=FALSE, warning=FALSE}
signature_txt = "CIBERSORT.DerM22.genes.txt"
cibersort_mat =
  CIBERSORT(
...
write_csv(cibersort_tbl, glue("{out_dir}/celltypes-cibersort-derm22.csv"))
cibersort_tbl
```
...
pheatmap(
  ...
  filename = glue("{out_dir}/celltypes-cibersort-derm22.heatmap.png"), width = 10, height = 8
)
pheatmap(
  cibersort_mat_filtered, border_color = NA, color = heatmap_colors,
  fontsize_row = 12, fontsize_col = 12,
  filename = glue("{out_dir}/celltypes-cibersort-derm22.heatmap.pdf"), width = 10, height = 8
)
```
...
sample_order =
  cibersort_tbl_long %>%
  filter(cell_type == "sc adipocytes") %>%
  arrange(-fraction) %>%
  pull(sample)
...
bar_plot = ggplot(cibersort_tbl_long) +
  ...
  scale_fill_igv()
save_plot(glue("{out_dir}/celltypes-cibersort-derm22.barplot.png"), plot = bar_plot, base_height = 6, base_width = 12)
save_plot(glue("{out_dir}/celltypes-cibersort-derm22.barplot.pdf"), plot = bar_plot, base_height = 6, base_width = 12)
bar_plot
```
